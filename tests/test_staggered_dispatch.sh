...
# + a READY task (so "nothing was claimed" assertions are not tautological —
# a paused board must leave a claimable task sitting ready).
# sets: BOARD_DB, QUOTA_TASK, CTRL_TASK, READY_TASK, BOARD=alpha
make_scratch_board() {
    BOARD=alpha
    ...
    READY_TASK="$("${envpin[@]}" "$REAL_HERMES" kanban create "claimable ready task" --assignee test-worker 2>/dev/null | grep -oE 't_[0-9a-f]{8}' | head -1)"
...
set_failures() {  # db task_id n  — fixture: pre-load a non-zero failure count
    python3 - "$1" "$2" "$3" <<'PY'
import sqlite3, sys
conn = sqlite3.connect(sys.argv[1])
conn.execute("UPDATE tasks SET consecutive_failures=? WHERE id=?", (int(sys.argv[3]), sys.argv[2]))
conn.commit()
PY
}
...
t7_sweep_mode_real_cli_requeues_quota_task() {
    new_env
    make_scratch_board
    # Pre-load a non-zero failure count so "not counted as a failure" is a real
    # assertion: the CLI's unblock_task resets consecutive_failures to 0
    # (hermes_cli/kanban_db.py unblock_task), i.e. it must never INCREASE it.
    set_failures "$BOARD_DB" "$QUOTA_TASK" 2
    ...
    assert_eq "$failures_before" "2" "t7: fixture actually had a non-zero failure count"
    assert_eq "$(task_status "$BOARD_DB" "$QUOTA_TASK")" "ready" "t7: quota-paused task re-queued to ready"
    assert_eq "$(task_failures "$BOARD_DB" "$QUOTA_TASK")" "0" "t7: re-queue is NOT counted as a failure (counter reset, never bumped)"
...
t9_full_run_real_cli_pause_no_spawn() {
    # real hermes binary, paused gate, and a task that IS claimable: nothing
    # must be invoked at all and the ready task must still be sitting ready
    # (otherwise "no claim" is vacuous — a board with only blocked tasks
    # cannot be claimed from anyway).
    new_env
    make_scratch_board
    local runs_before
...
PY
    ...
    assert_eq "$(task_status "$BOARD_DB" "$READY_TASK")" "ready" "t9: claimable task still ready (paused board cannot claim)"
...
PY
    ...
    assert_contains "$(marker_field alpha reason)" "zai-503-outage" "t9: pause reason recorded on the marker"
}
...
t16_empty_resume_at_does_not_shift_fields() {
    # regression (cross-family review finding): rate_limit_gate.py emits
    # resume_at: null on quota pauses. With a TAB-IFS read the empty field
    # collapsed, so the reason landed in GATE_RESUME_AT and GATE_REASON came
    # back EMPTY — the marker lost its reason and the 2h manager alert read
    # reason=''. Fields are now US(0x1f)-delimited, which preserves empties.
    new_env
    write_gate true null "zai-503-outage: resume_at null, reason must survive"
    run_dispatch
    assert_file_exists "$(marker_for alpha)" "t16: marker written with null resume_at"
    assert_eq "$(marker_field alpha reason)" "zai-503-outage: resume_at null, reason must survive" \
        "t16: reason preserved when resume_at is empty"
    assert_eq "$(marker_field alpha resume_at)" "None" "t16: resume_at stays null"
    assert_contains "$OUT" "reason=zai-503-outage: resume_at null, reason must survive" \
        "t16: log line carries the reason (not shifted into resume_at)"
